...
def compute_information_accretion(train_terms: Path, ontology_obo: Path, out_path: Path) -> Path:
    ...
    for aspect, namespace in ASPECT_TO_NS.items():
        graph = prune_orphans(build_ontology_graph(obo.edges_by_ns[namespace], obo.alt_to_canon))
