...
def embedding_dirs(dataset: DatasetSpec, spec: PredictorSpec) -> tuple[Path, ...]:
    return tuple(dataset.embedding_cache_dir / name for name in spec.embeddings)
...
def predict_predictor(
    ...
    for aspect in spec.min_freq:
        ...
        for batch_no, start in enumerate(range(0, n_seq, batch_size), start=1):
            end = min(start + batch_size, n_seq)
            seq_batch = seq_keys_x[start:end]
            x_batch = x[start:end]
            ...
            all_ids[offset:offset + n] = entry_ids
            all_pos[offset:offset + n] = topk_pos
        ...
        del all_ids, all_pos, all_scores
        gc.collect()
